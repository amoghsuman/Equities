            ...
            for beme, beme_portfolios in self.beme_dict.items():
                for portfolio in size_portfolios:
                    if portfolio in beme_portfolios:
                        ...
                        self.weights[portfolio] = 0    # Form the portfolios monthly as intersections of 2 portfolios formed on size and 3 portfolios formed on the ratio of book equity to market equity
            ...
            for size, size_portfolios in self.size_dict.items():
                for beme, beme_portfolios in self.beme_dict.items():
                    ...
                    if len(portfolios_intersection) == 2:
                        ...
                        long = self.portfolio[small_portfolio]                        short = self.portfolio[big_portfolio]
...
for ticker in TICKERS:
    ...
    cerebro.adddata(data)cerebro.addstrategy(FamaFrench,
                    size_dict={"S": ["S1", "S2", "S3"], "B": ["B1", "B2", "B3"]},
                    beme_dict={"L": ["L1", "L2", "L3"], "H": ["H1", "H2", "H3"]})
cerebro.run()
